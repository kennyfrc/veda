  ...
  return {
    ...
    async *run(input: I): AsyncIterable<PipelineEvent<O>> {
      ...
      for (const stage of stages) {
        ...
        if (result.error) {
          yield { type: 'error', stage: getStateName(stage), content: result.error, timestamp: Date.now() };
          break;
        }
        
        currentOutput = result.output;
        if (result.usage) usages.push(result.usage);
        context.priorSteps.push({ name: getStateName(stage), output: result.output });
      }
      
      yield { type: 'complete', output: currentOutput as O, totalUsage: combineUsage(usages), timestamp: Date.now() };
    },
  };
}
...
async function* executeStage(
  ...
  yield { type: 'stage_start', stage: stageName, timestamp: Date.now() };
  ...
  try {
    ...
    return { output: input, error: error instanceof Error ? error.message : String(error) };
  }
}

async function executeStep(step: Step<unknown, unknown>, input: unknown, context: StepContext): Promise<StageResult> {
  const result = await step.run(input, context);
  return { output: result.output, usage: result.usage };
}

async function executeEnsemble(ensemble: Ensemble<unknown, unknown>, input: unknown, context: StepContext): Promise<StageResult> {
  const result = await ensemble.run(input, context);
  return { output: result.selected, usage: result.usage };
}
...
async function* executeBranch(
  ...
  if (!branchStages) return { output: input, error: `Unknown branch: ${branchName}` };
  ...
  return { output: currentOutput, usage: combineUsage(usages) };
}
...
async function* executeLoop(
  ...
  while (iteration < stage.maxIterations && !stage.until(context)) {
    for (const bodyStage of stage.body) {
      ...
      currentOutput = result.output;
      if (result.usage) usages.push(result.usage);
      context.priorSteps.push({ name: `${stage.name}-iter-${iteration}`, output: result.output });
    }
    iteration++;
  }
